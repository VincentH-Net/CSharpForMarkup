...
using static WinUICsMarkupExamples.SearchViewModel;
using static WinUICsMarkupExamples.Styles;
using System;
...
public partial class SearchPage
{
    public void Build() => Content =
        StackPanel(
            ...
            SearchResults,
            Footer
        ).UI; // TODO: Uno has object Content instead of view type so no implicit conversion. Eliminate .UI somehow.

    // TODO: get documentation to be included in NuGet
    ...
    ListView SearchResults => ListView(() =>
        Grid(
            ...
            LikeButton ()
                        ...
                        .BindCommand (vm.LikeCommand, source: vm)
    ...
    static Ellipse RoundImage(double size) => Ellipse()
        .Width (size) .Height (size) .Fill (ImageBrush().ImageSource().Bind(tweet.AuthorImage));

    static void Format(UI.Controls.TextBlock textBlock, Tweet tweet)
    {
        textBlock.Inlines.Clear();
        tweet?.Body?.ForEach(fragment => textBlock.Inlines.Add(fragment.IsMatch ?
            Link (fragment.Text, vm.OpenTwitterSearchUri) .FontSize (15) .UI :
    ...
    static Button LikeButton() => Button()
        ...
        .Content().Bind(tweet.IsLikedByMe, convert: (bool like) => like ? "\u2764" : "\u2661"); // TODO: why does image not change on UWP? Command is invoked, converter not?

    static Hyperlink Link(string text, Uri uri) => Hyperlink (uri, text) .Foreground (CornflowerBlue);
    
    TextBlock Footer => TextBlock (Inlines(
		Run  ("See "),
        Link ("C# Markup", vm.OpenHelpUri),
		Run  (" and "),
		Link ("C# Markup Part 2", vm.OpenHelp2Uri),
